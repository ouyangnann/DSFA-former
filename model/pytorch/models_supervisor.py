...
from model.pytorch.dsfa import dsfa_former
...
class ModelsSupervisor:
    def __init__(self, models, load_pretrained, pretrained_model_dir, config_filename,cuda,**kwargs):
        ...
        self.batch_size = kwargs['data'].get('batch_size')
        #config information
        ...
        amodel = init_model(self._logger, self.graph, cuda,**self._model_kwargs)
    ...
    def _get_log_dir(loadmodel, config_name,kwargs):
        ...
        if log_dir is None:
            ...
            dataset_dir = kwargs['data'].get('dataset_dir')
    ...
    def _train(self, base_lr, stop_patience, steps, epochs, log_every=1, save_model=1,
        ...
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=steps, gamma=kwargs['lr_ratio'])
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            if epoch_num<10:
                ...
                self._logger.info(f'Iteration time: {np.mean(iter):.4f} seconds')  
